...
var __require = import.meta.require ?? createRequire(import.meta.url);
...
var NODE_USIZE_UNSUPPORTED = "Node FFI backend does not support u64 until (yet)";
...
function loadBackend() {
  ...
  try {
    const nodeFfi = requireModule("./ffi-adapter.cjs");
    return createNodeBackend(nodeFfi.default ?? nodeFfi);
  } catch {
    return createUnsupportedBackend(new Error("FFI backend not available"));
  }
}
...
function ffiBool(value) {
  // HarmonyOS (koffi) requires actual Boolean type for native bool params
...
function createNodeBackend(nodeFfi) {
  return {
    ...
    toArrayBuffer(pointer, offset, length) {
      return nodeFfi.toArrayBuffer(toBigIntPointer(pointer), offset, length);
    }
  };
}
...
function importModule(specifier) {
  if (specifier === "node:ffi") {
    ...
    // 从当前目录向上查找 polyfill/node-ffi-koffi.cjs
    let base = dirname(fileURLToPath(import.meta.url));
    while (base !== dirname(base)) {
      const candidate = join(base, "polyfill", "node-ffi-koffi.cjs");
      if (fs.existsSync(candidate)) {
        return import(candidate).then((module) => module.default ?? module);
      }
      base = dirname(base);
    }
    // 最后一层 fallback（当向上查找失败时）
    throw new Error("Cannot find polyfill/node-ffi-koffi.cjs from " + import.meta.url);
  }
...
function createNodeBackend2(nodeFfi) {
  return {
    ...
    toArrayBuffer(pointer, offset, length) {
      return nodeFfi.toArrayBuffer(toBigIntPointer2(pointer), offset, length);
    }
  };
}
...
// src/zig.ts
var _nativePlatform = process.platform;
var nativePackage = await import(`@opentui/core-${_nativePlatform}-${process.arch}`);
...
function getOpenTUILib(libPath) {
  ...
  if (process.env.OPENTUI_DEBUG) console.error("[OpenTUI FFI] dlopen path:", resolvedLibPath);
  const rawSymbols = dlopen(resolvedLibPath, {
    ...
    bufferDrawSuperSampleBuffer: {
      args: ["ptr", "u32", "u32", "ptr", "u64", "u8", "u32"],
      returns: "void"
    },
    bufferDrawPackedBuffer: {
      args: ["ptr", "ptr", "u64", "u32", "u32", "u32", "u32"],
      returns: "void"
    },
    ...
    textBufferGetTextRange: {
      args: ["ptr", "u32", "u32", "ptr", "u64"],
      returns: "u64"
    },
    textBufferGetTextRangeByCoords: {
      args: ["ptr", "u32", "u32", "u32", "u32", "ptr", "u64"],
      returns: "u64"
    },
...
// Module-level keepalive to prevent GC of koffi callback trampolines
const _ffiCallbackKeepalive = [];
...
class FFIRenderLib {
  ...
  setupLogging() {
    ...
    const logCallbackFn = (level, msgPtr, msgLenBigInt) => {
      try {
    ...
    const logCallback = this.opentui.createCallback(logCallbackFn, {
      args: ["u8", "ptr", "u64"],
      returns: "void"
    });
    this.logCallbackWrapper = logCallback;
    _ffiCallbackKeepalive.push(logCallbackFn);
    if (!logCallback.ptr) {
  ...
  setupEventBus() {
    ...
    const eventCallbackFn = (namePtr, nameLenBigInt, dataPtr, dataLenBigInt) => {
      try {
    ...
    const eventCallback = this.opentui.createCallback(eventCallbackFn, {
      args: ["ptr", "u64", "ptr", "u64"],
      returns: "void"
    });
    this.eventCallbackWrapper = eventCallback;
    _ffiCallbackKeepalive.push(eventCallbackFn);
  ...
  ensureNativeSpanFeedCallback() {
    ...
    const callbackFn = (streamPtr, eventId, arg0, arg1) => {
      const handler = this.nativeSpanFeedHandlers.get(streamPtr);
      if (handler) {
        handler(eventId, arg0, arg1);
      }
    };
    const callback = this.opentui.createCallback(callbackFn, {
      args: ["ptr", "u32", "ptr", "u64"],
      returns: "void"
    });
    this.nativeSpanFeedCallbackWrapper = callback;
    _ffiCallbackKeepalive.push(callbackFn);
    if (!callback.ptr) {
  ...
  requestRender() {
    ...
    if (this._isRunning) {
      if (!this.immediateRerenderRequested) {
        const now = Date.now();
        if (!this.__lastImmediateTime || now - this.__lastImmediateTime > 32) {
          this.immediateRerenderRequested = true;
          this.__lastImmediateTime = now;
        }
      }
      return;
    }
    if (this._controlState !== "explicit_started" && this._controlState !== "auto_started") {
      return;
    }
    if (this.rendering) {
      this.immediateRerenderRequested = true;
...
`.repeat(space);
  ...
  setupInput() {
    ...
    if (this.stdin.setRawMode) {
      try { this.stdin.setRawMode(true); } catch(e) {}
    }
    ...
    try {
      ...
      if (!this._isDestroyed) {
        ...
        if (this._isRunning || this.immediateRerenderRequested) {
          ...
          const delay = Math.max(8, targetFrameTime - Math.floor(overallFrameTime));
          this.immediateRerenderRequested = false;
          this.renderTimeout = this.clock.setTimeout(() => {
            this.renderTimeout = null;
            this.loop();
          }, delay);
          // Ensure timer keeps event loop alive on platforms where setTimeout may not ref() by default
          if (this.renderTimeout && typeof this.renderTimeout.ref === "function") {
            try { this.renderTimeout.ref(); } catch (e) { /* ignore */ }
          }
        } else {
          this.clock.clearTimeout(this.renderTimeout);
          this.renderTimeout = null;
