// @bun
// index.ts
// Polyfill global DOM types needed by solid-js client runtime
// domino is bundled in the main output but doesn't set up globals automatically
...
function createPolyfillNode(tagName, nodeType, nodeValue) {
  const children = [];
  return {
    tagName: tagName || "",
    nodeName: tagName || "",
...
if(typeof globalThis.document === "undefined") {
  ...
  globalThis.document = {
    createElement: (tagName) => createPolyfillNode(tagName, 1, null),
    createElementNS: (ns, tagName) => createPolyfillNode(tagName, 1, null),
    createTextNode: (value) => createPolyfillNode("#text", 3, value),
    createComment: (value) => createPolyfillNode("#comment", 8, value),
    createDocumentFragment: () => createPolyfillNode("#document-fragment", 11, null),
...
// Make window.document resolve to our polyfill (needed by solid-js delegateEvents)
if(typeof globalThis.window === "undefined") {
  globalThis.window = globalThis;
}
// Polyfill requestAnimationFrame for SolidJS (needed before CliRenderer sets its own)
...
function createRenderer({
  ...
  function insert(parent, accessor, marker, initial) {
    ...
    createEffect((current) => {
      var accResult = accessor();
      var result = insertExpression(parent, accResult, current, marker);
      return result;
    }, initial);
  }
...
var mountSolidRoot = (renderer, node) => {
  ...
  try {
    console.log('[SolidRender] mountSolidRoot calling _render with root:', typeof renderer.root, 'rootType:', renderer.root?.constructor?.name);
    const _renderStart = Date.now();
    var _mountCode = () => createComponent2(RendererContext.Provider, {
      get value() {
        return renderer;
      },
      get children() {
    ...
    dispose = _render(_mountCode, renderer.root);
    console.log('[SolidRender] mountSolidRoot _render done in', Date.now() - _renderStart, 'ms');
  } finally {
    mounting = false;
...
var render = async (node, rendererOrConfig = {}) => {
  ...
  try {
    ...
    createRoot((rootDispose) => {
      ...
      renderer.once("destroy", rootDispose);
    });
    renderer.start();
  } catch (e) {
    throw e;
