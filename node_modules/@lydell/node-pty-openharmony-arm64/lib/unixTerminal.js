...
var UnixTerminal = /** @class */ (function (_super) {
    ...
    function UnixTerminal(file, args, opt) {
        ...
        env.TERM = name;
        // OHOS: LD_PRELOAD preload_tty.so for TTY emulation
        if (process.platform === 'openharmony') {
            var preloadPath = path.resolve(__dirname, native.dir, 'preload_tty.so');
            env.LD_PRELOAD = preloadPath;
            env.PRELOAD_PTY_COLS = String(_this._cols);
            env.PRELOAD_PTY_ROWS = String(_this._rows);
        }
        ...
        if (term.pty === '') {
            // OHOS socketpair fallback - use net.Socket instead of tty.ReadStream
            _this._socket = new net.Socket({ fd: term.fd });
        } else {
        ...
        _this._writeStream = new CustomWriteStream(term.fd, (encoding || undefined), term.pid, term.pty === '');
        // setup
        ...
        _this._socket.on('close', function () {
            ...
            _this._emittedClose = true;
            // Socketpair mode: kernel won't send SIGHUP on master close
            if (_this._pty === '' && _this._pid > 0) {
                try { process.kill(_this._pid, 'SIGHUP'); } catch (e) {}
            }
            _this._close();
            _this.emit('close');
        });
    ...
    UnixTerminal.prototype.resize = function (cols, rows, pixelSize) {
        ...
        if (this._pty !== '') {
            ...
            // Socketpair mode: send SIGWINCH so child can query new size
            try { process.kill(this._pid, 'SIGWINCH'); } catch (e) {}
        }
        this._cols = cols;
        this._rows = rows;
    };
...
var CustomWriteStream = /** @class */ (function () {
    function CustomWriteStream(_fd, _encoding, _pid, _signalForward) {
        this._fd = _fd;
        this._encoding = _encoding;
        this._pid = _pid;
        this._signalForward = _signalForward;
        this._writeQueue = [];
    }
    ...
    CustomWriteStream.prototype.write = function (data) {
        ...
        // In socketpair mode, forward control chars as signals
        // (mimics PTY line discipline behaviour)
        if (this._signalForward && this._pid > 0) {
            for (var i = 0; i < buffer.length; i++) {
                switch (buffer[i]) {
                    case 0x03: // Ctrl+C
                        try { process.kill(this._pid, 'SIGINT'); } catch (e) {}
                        break;
                    case 0x1A: // Ctrl+Z
                        try { process.kill(this._pid, 'SIGTSTP'); } catch (e) {}
                        break;
                    case 0x1C: // Ctrl+\
                        try { process.kill(this._pid, 'SIGQUIT'); } catch (e) {}
                        break;
                }
            }
        }
